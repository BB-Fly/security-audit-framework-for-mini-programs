...
if args.randomUserAgent:
    try:
        ...
        with open(_dir + "user-agents.txt","r") as fd:
            nb = 0
            for l in fd:
                nb += 1
            fd.seek(0)
...
try:
    ...
    with open(_dir + "techniques.json", "r") as rawTechniques:
        techniques = json.loads(rawTechniques.read())
...
for template in templates:
    try:
        ...
        with open(os.path.join((_dir + templatesFolder + "/" + template["filename"])), 'rb') as templatefd:
            templatesData[template["templateName"]] = templatefd.read()
